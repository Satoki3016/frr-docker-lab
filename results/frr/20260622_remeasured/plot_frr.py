...
def compare_rtt(active):
    ...
    fig.suptitle(
        f"片道遅延 ({metric}) 比較  (FRR OSPF-SR + DiffServ-TE)\n"
        "時計ドリフト補正済み (AF41 実測値を基準に差分除去)",
        fontsize=13, fontweight="bold", y=1.01)

    # AF41 OWD を各シナリオの時計ドリフト基準として先読み
    af41_refs: dict[str, tuple] = {}
    for d, n, c, ls, _ in active:
        p = _find_owd(BASE / d, 1)
        if p:
            t41, v41 = _load_owd(p)
            if t41:
                af41_refs[n] = (np.array(t41, dtype=float),
                                np.array(v41, dtype=float))
    ...
    for ax, (pri_label, pri_color, col, tx) in zip(axes, PRIORITIES):
        ...
        for d, n, c, ls, _ in active:
            ...
            # 時計ドリフト補正: AF41 の実測 OWD (≈ 真のドリフト量) を差し引く
            # 補正後: AF41≈0ms, AF42≈10ms(netem), AF43≈40ms(netem)
            if n in af41_refs:
                t_ref, v_ref = af41_refs[n]
                drift = np.interp(t_arr, t_ref, v_ref,
                                  left=v_ref[0], right=v_ref[-1])
                v_arr = v_arr - drift

            if len(v_arr):
                ax.plot(t_arr, v_arr, label=n, color=c, linestyle=ls, alpha=0.85)
                all_vals.extend(v_arr.tolist())

        if all_vals:
            p95 = float(np.percentile(all_vals, 95))
            ax.set_ylim(bottom=max(-2.0, min(all_vals) - 2),
                        top=max(p95 * 1.3, 2.0))
