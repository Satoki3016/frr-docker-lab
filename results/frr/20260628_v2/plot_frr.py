...
_parser.add_argument("--base", type=str, default=None,
                     help="実験フォルダのパス (デフォルト: plot_frr.py の親ディレクトリ)")
...
# スループット単位: 1 Gbps 以上なら Gbps、それ以外は Mbps
...
def compare_throughput(active):
    ...
    for ax, (pri_label, pri_color, col, _), wrr_target in zip(axes, PRIORITIES, WRR_TARGETS):
        for d, n, c, ls, _ in active:
            ...
            if t:
                ax.plot(t, [v * THRU_SCALE for v in mb], label=n, color=c, linestyle=ls)
        ax.axhline(wrr_target * THRU_SCALE, color=pri_color, ls=":", lw=1.5, alpha=0.6,
                   label=f"WRR目標 ({wrr_target * THRU_SCALE:.2f} {THRU_UNIT})")
        ax.set_xlim(0, PLOT_END); ax.set_ylim(bottom=0)
        ax.set_ylabel(f"スループット ({THRU_UNIT})", fontsize=12)
