...
各CRリンク（10G）のHTB構成:
1:0  root  rate=10G
 ├── 1:1  AF41  prio=0  rate=10%×10G=1G           ceil=10G  (SP優先)
 ├── 1:2  AF42  prio=1  rate=2/7×10G/10≈0.29G     ceil=10G  (WRR 2)
 └── 1:3  AF43  prio=1  rate=1/7×10G/10≈0.14G     ceil=10G  (WRR 1)
```
...
（保証rateをフルシェア 2/7・1/7 のままにすると保証合計5.29Gが先に消費され、
AF41の上限が 1G+4.71G=5.71G となり8Gに到達できない — 2026-07-03実測で確認済み）

HTBのquantumが AF41:AF42:AF43 = 4:2:1（36K:18K:9K bytes/round）のため、
全クラスが輻輳する状況では帯域が 4:2:1 で分配される。

### 送信レート設定（lab_config.sh）

iperf3は `-P 4 -b TX_RATE` で4ストリーム送信するため、総送信量 = 4 × TX_RATE。

```
TX1_RATE=2G     # AF41: 4×2G=8G < リンク10G → SP最優先で輻輳下でも極力ロスなし
TX2_RATE=2G     # AF42: 4×2G=8G >> 2/7×10G=2.86G → 意図的輻輳（超過率2.8倍）
TX3_RATE=2G     # AF43: 4×2G=8G >> 1/7×10G=1.43G → 意図的輻輳（超過率5.6倍）
CR1_BW=CR2_BW=CR3_BW=10G
# 総送信量: 8G+8G+8G=24G >> 10G（2.4倍オーバーサブスクリプション）
```

**期待する計測結果（normal シナリオ）:**
- AF41: ≈8 Gbps、損失ほぼゼロ（リンク10G以内 → SP保護で全量通過）
- AF42: ≈1.33 Gbps（残余帯域 2G を WRR 2:1 で分配、損失≈83%）
- AF43: ≈0.67 Gbps（同上、損失≈92%）
