...
# enp23s0f1np1: Tx1 (AF41 高優先)
ip netns exec LER_Ingress_ns tc qdisc add dev enp23s0f1np1 handle ffff: ingress
ip netns exec LER_Ingress_ns tc filter add dev enp23s0f1np1 parent ffff: protocol all \
    u32 match u32 0 0 \
    police rate "$ingress_hi" burst "${pb_hi}b" drop flowid :1

# enp179s0f1np1: Tx2 (AF42 中優先)
ip netns exec LER_Ingress_ns tc qdisc add dev enp179s0f1np1 handle ffff: ingress
ip netns exec LER_Ingress_ns tc filter add dev enp179s0f1np1 parent ffff: protocol all \
    u32 match u32 0 0 \
    police rate "$ingress_me" burst "${pb_me}b" drop flowid :1

# enp5s0f1: Tx3 (AF43 低優先)
ip netns exec LER_Ingress_ns tc qdisc add dev enp5s0f1 handle ffff: ingress
ip netns exec LER_Ingress_ns tc filter add dev enp5s0f1 parent ffff: protocol all \
    u32 match u32 0 0 \
    police rate "$ingress_lo" burst "${pb_lo}b" drop flowid :1

echo "  総帯域: ${total_bw_tc}"
echo "  enp23s0f1np1  (Tx1/AF41 高優先): police rate ${ingress_hi} burst ${pb_hi}b"
echo "  enp179s0f1np1 (Tx2/AF42 中優先): police rate ${ingress_me} burst ${pb_me}b"
echo "  enp5s0f1      (Tx3/AF43 低優先): police rate ${ingress_lo} burst ${pb_lo}b"
