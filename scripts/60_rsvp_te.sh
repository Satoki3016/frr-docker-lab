...
# ICMP: 入力インターフェースでDSCP設定（物理NIC/veth 自動判別）
if ip netns exec LER_Ingress_ns ip link show enp23s0f1np1 2>/dev/null | grep -q "enp23s0f1np1"; then
    _TX1_IF="enp23s0f1np1"; _TX2_IF="enp179s0f1np1"; _TX3_IF="enp5s0f1"
else
    _TX1_IF="leri-tx1"; _TX2_IF="leri-tx2"; _TX3_IF="leri-tx3"
fi
ip netns exec LER_Ingress_ns iptables -t mangle -A PREROUTING \
    -i "$_TX1_IF" -p icmp -j DSCP --set-dscp-class AF41
ip netns exec LER_Ingress_ns iptables -t mangle -A PREROUTING \
    -i "$_TX2_IF" -p icmp -j DSCP --set-dscp-class AF42
ip netns exec LER_Ingress_ns iptables -t mangle -A PREROUTING \
    -i "$_TX3_IF" -p icmp -j DSCP --set-dscp-class AF43
