...
if ip netns exec LER_Ingress_ns ip link show enp23s0f1np1 2>/dev/null | grep -q "enp23s0f1np1"; then
    TX1_DEV="enp23s0f1np1"; TX2_DEV="enp179s0f1np1"; TX3_DEV="enp5s0f1"
else
    TX1_DEV="leri-tx1"; TX2_DEV="leri-tx2"; TX3_DEV="leri-tx3"
fi
declare -A _DEV_CLASS=(["$TX1_DEV"]="AF41(高)" ["$TX2_DEV"]="AF42(中)" ["$TX3_DEV"]="AF43(低)")
for dev in "$TX1_DEV" "$TX2_DEV" "$TX3_DEV"; do
    cls="${_DEV_CLASS[$dev]}"
